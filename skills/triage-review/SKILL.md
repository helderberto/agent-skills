...
name: triage-review
effort: high
description: Fetch unresolved review comments on a GitHub PR, verify each against the current code, and classify as Address / Skip / Optional / Discuss with a one-line rationale. Use when the user asks which review comments to address or wants to sort signal from noise on a PR. Don't use to reply to or resolve threads (read-only) or to write a fresh review (/code-review).
